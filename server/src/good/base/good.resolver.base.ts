...
import { GoodFindManyArgs } from "./GoodFindManyArgs";
import { GoodFindUniqueArgs } from "./GoodFindUniqueArgs";
...
export class GoodResolverBase {
  ...
  async good(
    @graphql.Args() args: GoodFindUniqueArgs,
    @gqlUserRoles.UserRoles() userRoles: string[]
  ): Promise<Good | null> {
    const permission = this.rolesBuilder.permission({
      role: userRoles,
